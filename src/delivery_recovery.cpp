...
std::string sessionDirectoryPath(uint32_t trackingSessionNumber) {
  char path[48];
  std::snprintf(path, sizeof(path), "/session-%010lu",
...
std::string sessionFilePath(uint32_t trackingSessionNumber,
                            const char* filename) {
  return sessionDirectoryPath(trackingSessionNumber) + "/" + filename;
}
...
void DeliveryRecovery::forgetSession(uint32_t trackingSessionNumber) {
  sessions_.erase(
      std::remove_if(sessions_.begin(), sessions_.end(),
                     [trackingSessionNumber](
                         const RecoveredTrackingSession& session) {
                       return session.trackingSessionNumber ==
                              trackingSessionNumber;
                     }),
      sessions_.end());
}
