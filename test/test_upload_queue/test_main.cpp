...
void no_pending_batch_is_distinct_from_storage_failures() {
  ...
  TEST_ASSERT_EQUAL_INT(
      static_cast<int>(tracking::BatchReadStatus::no_pending_batch),
      static_cast<int>(tracking::readPendingBatch(
...
void oldest_pending_batch_sets_identity_and_range() {
  std::vector<tracking::RecoveredTrackingSession> sessions;
...
void inactive_partial_batch_reads_only_its_pending_points() {
  std::vector<tracking::RecoveredTrackingSession> sessions;
  sessions.push_back(session(4, 34, 30, true));
  FakePointSource source;
  source.returnedLineCount = 4;
  tracking::UploadBatch batch;
  ...
  TEST_ASSERT_EQUAL_UINT32(4, source.requestedCount);
  TEST_ASSERT_EQUAL_UINT32(4, batch.ndjsonPoints.size());
}
...
int main(int, char**) {
  UNITY_BEGIN();
  RUN_TEST(no_pending_batch_is_distinct_from_storage_failures);
  ...
  RUN_TEST(oldest_pending_batch_sets_identity_and_range);
  RUN_TEST(inactive_partial_batch_reads_only_its_pending_points);
  return UNITY_END();
}
